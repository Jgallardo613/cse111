...
def pressure_loss_from_pipe(pipe_diameter, pipe_length, friction_factor, fluid_velocity):
    return -friction_factor * WATER_DENSITY * (fluid_velocity ** 2) * (pipe_length / (2 * pipe_diameter * 1000))
...
def pressure_loss_from_pipe_reduction(larger_diameter, fluid_velocity, reynolds_number_value, smaller_diameter):
    k = 0.1 + (50 / reynolds_number_value) * ((larger_diameter / smaller_diameter) ** 4 - 1)
